...
import { getAudioRootPath, getChapterAudio } from '@/lib/audio'
...
export async function GET(_request: Request, context: RouteContext) {
  ...
  const chapterAudio = await getChapterAudio(normalizedBook, chapterNumber)

  if (!chapterAudio) {
    return new NextResponse('Audio not found', { status: 404 })
  }

  const audioRoot = getAudioRootPath()
